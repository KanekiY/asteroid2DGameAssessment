...
ErrorType Game::StartOfGame()
{
   ...
   // **********************************************************************
	...
	lvlManger = new LevelManager();
	lvlManger->initialise(om);
	om->lvlM = lvlManger;
	lvlManger->startOfGame();
	MySoundEngine* pBG = MySoundEngine::GetInstance();
	bgmusic = pBG->LoadWav(L"hiss.wav");
	pBG->Play(bgmusic);
...
ErrorType Game::Update()
{
   ...
   // *********************************************************************
	...
	lvlManger->update(gt.mdFrameTime);
	//om->deleteAll();
